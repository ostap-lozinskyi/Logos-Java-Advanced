...
<body style="background: url(/resources/img/fon2.jpg)">
	<div class="container" class="container" style="background-color: white;">
		<div class="row">
			<div class="col-12">
				<h1 class="text-center">Order${placeCurrent.id}</h1>
			</div>
		</div>
		<div class="row">
			<div class="col-12">
				...
				<form:form action="/place/${placeCurrent.id}/order" method="POST" modelAttribute="order">
					...
					<div class="form-group row">
						<div class="col-8 mr-auto">
							...
							<a href="/place/${placeCurrent.id}/order<custom:allParams/>" class="btn btn-sm btn-outline-warning">Cancel</a>
						</div>
					</div>
				</form:form>
			</div>
					...
					<c:forEach var="order" items="${orders.content}">
						<tr>
							...
							<td class="text-center">
								<a	href="/place/${placeCurrent.id}/order/update/${order.id}<custom:allParams/>" class="btn btn-outline-warning btn-sm">Update</a>
								<a	href="/place/${placeCurrent.id}/order/delete/${order.id}<custom:allParams/>" class="btn btn-outline-danger btn-sm">Delete</a>
							</td>
						</tr>
					</c:forEach>
				</table>
